## 0.12.0 (October 24, 2014)

### React Core

#### Breaking Changes

* `key` and `ref` moved off props object, now accessible as `instance.key` and `instance.ref` instead
* React is now BSD licensed with accompanying Patents clause
...
* Removed DevTools message when the DevTools are installed (requires update to DevTools, should auto-update)
...
* Enforced convention: lower case tag names are always treated as HTML tags, upper case tags names are always treated as composite components
...
* Make sourcemaps an option when using JSXTransformer APIs directly (eg, for react-rails)
